# smth
用来抓取水木社区 FamilyLife 版面的帖子。`smth_scraper.py` 会遍历版面页、进入每个主题、解析 `发信人` 与 `[FROM: …]` 中的 IP，并把结果写入 CSV。
...
- `end_page`：结束的版面页码，默认 100，表示会抓取 1~100 页。
...
# 指定页码范围并只记录某个 ID，可覆盖配置文件（同时把延迟固定成 1 秒）
python smth_scraper.py --start-page 3 --end-page 5 --id wpn --id anotherID --delay 1
...
- `ips`：在该楼层内所有 `font.f005` 中 `[FROM:…]` 的内容，使用 `|` 连接。

可以通过 `--output` 更改结果文件；`--delay`（仅数字）会把等待时间固定为指定秒数，留空则采用配置文件或默认的 2~5 秒随机间隔；`--max-threads` 可覆盖配置中的主题数量限制，用于调试或采样。
